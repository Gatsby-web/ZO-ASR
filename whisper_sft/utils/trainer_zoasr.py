...
class OurTrainer(Seq2SeqTrainer):
    ###
    def __init__(self, *args, custom_trainer=None, custom_eps=None, log_path=None, q=2, seed=42, start_layer=0, end_layer=-1, **kwargs):
        super().__init__(*args, **kwargs)
        self.custom_trainer = custom_trainer
        self.zo_eps = custom_eps
    ...
    def _inner_training_loop(
        ...
        for those updates.
        """
        logger = setup_logging(self.log_path)
                ...
                if (
                    ...
                    if self.state.global_step%5 == 0:
                        with open(self.log_path, 'a') as f:
                            f.write(f"Step: {self.state.global_step}, Loss: {tr_loss.item()/5}\n")
                        #wandb.log({"loss": tr_loss.item()/5, "step": self.state.global_step})
    ...
    def zo_step(self, model, inputs):
        """
        Perform ZO-ASR gradient estimation with multiple queries. Returns the average loss from perturbations.
        """
        args = self.args
        ...
        if self.end_layer == -1:
            self.end_layer = len(self.named_parameters_to_optim)
        self.named_parameters_to_optim = self.named_parameters_to_optim[self.start_layer : self.end_layer]

        seeds = []
        projected_grads = []
        loss_sum = 0.0

        for _ in range(self.q):  # q query
    ...
    def zo_update(self, model):
        """
        Apply parameter updates using the estimated gradients from ZO-ASR, averaging over n steps.
        """
        args = self.args
    ...
    def save_model(self, output_dir=None, _internal_call=False):
        ...
        save_path = os.path.join(output_dir, f"checkpoint-{steps}.pt")
